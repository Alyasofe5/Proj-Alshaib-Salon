            ...
            <div className="topbar">
                ...
                <div className="flex gap-2">
                    {(["daily", "monthly", "yearly"] as Period[]).map((p) => (
                        <button
                            key={p}
                            className={`px-4 py-2 rounded-lg text-sm font-bold transition-all ${period === p ? "btn-lime" : "btn-outline-lime"}`}
                            onClick={() => setPeriod(p)}
                        >
            ...
            <div className="content-area">
                ...
                {period === "monthly" && data && (
                    <>
                        <div className="grid grid-cols-2 md:grid-cols-2 lg:grid-cols-4 gap-3 mb-6">
                            ...
                            <StatCard icon={<Banknote size={18} />} value={Number((data as { total_sales: number }).total_sales).toFixed(3)} label="إجمالي المبيعات" color="lime" />
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-1 lg:grid-cols-3 gap-4">
                            <div className="lg:col-span-2 chart-card">
                                <div className="chart-card-title flex items-center gap-2">
                                    <BarChart3 size={15} color="#C3D809" /> المبيعات <span>اليومية</span>
                                </div>
                                <ResponsiveContainer width="100%" height={220}>
                                    <LineChart data={((data as { chart: { labels: number[]; data: number[] } }).chart?.labels || []).map((d: number, i: number) => ({ day: d, sales: ((data as { chart: { data: number[] } }).chart?.data || [])[i] }))} margin={{ top: 10, right: 0, left: 0, bottom: 0 }}>
                                        ...
                                        <YAxis orientation="right" tick={{ fill: "var(--color-text-muted)", fontSize: 11 }} width={30} />
                                        <Tooltip contentStyle={{ background: "var(--color-surface)", border: "1px solid #333", borderRadius: 8 }} />
                                        <Line type="monotone" dataKey="sales" stroke="var(--color-accent)" strokeWidth={2} dot={{ fill: "var(--color-accent)", r: 3 }} />
                                    </LineChart>
                                </ResponsiveContainer>
                            </div>
                ...
                {period === "yearly" && data && (
                    <>
                        ...
                        <div className="grid grid-cols-2 md:grid-cols-2 lg:grid-cols-4 gap-3 mb-6">
                            <StatCard icon={<Users size={18} />} value={(data as { total_customers: number }).total_customers} label="إجمالي الزبائن" color="blue" />
                            <StatCard icon={<Banknote size={18} />} value={Number((data as { total_sales: number }).total_sales).toFixed(3)} label="إجمالي المبيعات (د.أ)" color="lime" />
                        ...
                        <div className="chart-card mb-6">
                            <div className="chart-card-title flex items-center gap-2">
                                <BarChart3 size={15} color="#C3D809" />
                            ...
                            <ResponsiveContainer width="100%" height={260}>
                                ...
                                >
                                    ...
                                    />
                                    <Legend formatter={(v) => v === "sales" ? "المبيعات" : "المصاريف"} wrapperStyle={{ color: "var(--color-text-muted)", fontSize: 12 }} />
                                    <Bar dataKey="sales" fill="var(--color-accent)" radius={[4, 4, 0, 0]} name="sales" />
